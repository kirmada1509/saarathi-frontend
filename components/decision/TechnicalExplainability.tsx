    ...
    >
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <Stack
              className="border-t border-border-default p-5 bg-bg-surface-raised/20"
              gap={4}
            >
              {children}
            </Stack>
...
export function TechnicalExplainability({
  trace,
  ranked,
  verdict,
}: TechnicalExplainabilityProps) {
      ...
      <Stack gap={6}>
        ...
        <Stack gap={1}>
          ...
          <Text variant="subtext" size="sm">
            Advanced internals — decision trace, ranking methodology, candidate
            flights, and counterfactual analysis.
          </Text>
        </Stack>
        ...
        <Stack gap={3}>
          ...
          {trace.length > 0 && (
            ...
            >
              <Text variant="subtext" size="xs" className="mb-3">
                Each stage represents one step in the deterministic AI decision
                pipeline. Click a stage to inspect its full JSON payload.
              </Text>
              <Stack direction="row" align="center" gap={2} className="flex-1">
                {trace.map((stage: TraceStage, index: number) => (
                  <>
                    ...
                    {index < trace.length - 1 && <ArrowRight />}
                  </>
                ))}
              </Stack>
            </AccordionSection>
          ...
          >
            <Stack gap={3}>
              ...
              >
                Saarathi uses a weighted multi-attribute scoring function where
                each flight is evaluated across six dimensions: fare price,
                route directness, connection convenience, cabin class alignment,
                airline loyalty, and baggage policy. Weights are inferred from
                the user&apos;s profile, historical behavior, and the parsed
                request text using evidence extraction.
              </Text>
          ...
          {ranked.length > 0 && (
            ...
            >
              <Text variant="subtext" size="xs" className="mb-3">
                Full ranked list of all flights evaluated. Click any row to see
                its score breakdown.
              </Text>
              <Stack
                className="border border-border-default rounded-xl overflow-hidden bg-bg-surface-raised/10"
                gap={0}
              >
                <Stack
                  className="grid grid-cols-5 gap-2 p-3 bg-bg-surface-raised/60 border-b border-border-default text-xs font-mono font-medium text-text-secondary"
                  gap={0}
                >
                ...
                <Stack gap={0} className="divide-y divide-border-default">
                  {ranked.map((f: ScoredFlight, index: number) => {
                    const isChampion =
                      verdict && f.flight_id === verdict.flight_id;
                    ...
                    return (
                      <Stack key={f.flight_id} gap={0}>
                        ...
                        >
                          <span className="col-span-2 flex items-center gap-2">
                            <Text
                              variant="mono"
                              size="xs"
                              className="text-text-secondary font-bold min-w-5"
                            >
                              #{index + 1}
                            </Text>
                            <Stack gap={1}>
                              ...
                              >
                                {f.airline_code} {f.flight_numbers} |{" "}
                                {f.cabin_class}
                              </Text>
                            </Stack>
                          </span>
                        ...
                        {isExpanded && f.breakdown && (
                          <Stack
                            className="p-4 bg-bg-surface-raised/60 border-t border-b border-border-default/60"
                            gap={4}
                          >
                            <Stack
                            ...
                            >
                              ...
                              <Stack gap={3}>
                                <Text
                                  variant="heading"
                                  size="xs"
                                  className="text-accent font-semibold flex items-center gap-1.5"
                                >
                                ...
                                <Stack gap={2}>
                                  {f.breakdown.dayBonus > 0 && (
                                    ...
                                    >
                                      ...
                                      <Text
                                        variant="mono"
                                        size="xs"
                                        className="text-signal-positive font-bold"
                                      >
                                  ...
                                  <Stack
                                    direction="row"
                                    justify="between"
                                    className="border-t border-border-default/60 pt-2 font-bold text-accent w-full"
                                    gap={0}
                                  >
          ...
          >
            <Text
              ...
              className="leading-relaxed text-text-primary/90"
            >
              Preferences are extracted from three sources: the traveler&apos;s
              structured profile (home airport, preferred cabin, airline
              loyalty), raw booking history (patterns inferred from past trips),
              and the natural language request text. Sources are combined and
              weighted by signal strength — matching evidence from multiple
              sources produces strong signals.
            </Text>
          </AccordionSection>
          ...
          >
            <Text
              ...
              className="leading-relaxed text-text-primary/90"
            >
              For each non-winning flight, Saarathi computes the exact price or
              preference change that would cause it to become the winner. Price
              thresholds are solved using closed-form algebra on the scoring
              function. Preference flips are computed by toggling individual
              constraints and re-running the score comparison. This reveals the
              decision boundary with mathematical precision.
            </Text>
          </AccordionSection>
        </Stack>
      </Stack>
      ...
      >
        <SheetContent className="w-full sm:max-w-xl bg-bg-surface border-l border-border-default text-text-primary p-6 overflow-y-auto z-9999">
          ...
          <Stack gap={4} className="mt-4">
            <Text variant="subtext" size="xs">
              The exact JSON payload evaluated or returned by this step in the
              deterministic decision core.
            </Text>
            {activeStage && (
              <Stack
                className="bg-bg-surface-raised border border-border-default rounded-lg p-4 overflow-x-auto max-h-[75vh]"
                gap={0}
              >
